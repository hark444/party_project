...
def create_party_attended_object(client, headers, party_obj):
    ...
    party_attended_obj = client.post(parties_attended_url, headers=headers, json=DEFAULT_PARTIES_ATTENDED_PAYLOAD)
...
def test_get_single_party_attended(client, account_user_and_token):
    ...
    party_obj = create_party_object(client, headers)
    party_attended_obj = create_party_attended_object(client, headers, party_obj)
    get_party_attended_url = parties_attended_url + f"/{party_attended_obj.json().get('id')}"
    response = client.get(get_party_attended_url, headers=headers)
    assert response.status_code == 200
    response_json = response.json()
    assert response_json.get("comment") == DEFAULT_PARTIES_ATTENDED_PAYLOAD.get("comment")
    assert response_json.get("approved") == DEFAULT_PARTIES_ATTENDED_PAYLOAD.get("approved")
